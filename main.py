...
async def _enter_app(from_user, answer_msg=None, edit_call=None):
    ...
    if active:
        ...
        cta = 'Всё готово к работе. Жми кнопку ниже 👇'
    else:
        status_ln = (
            f'<blockquote>🆓 Старт бесплатный. Подключи аккаунт — '
            f'и инструменты разблокируются прямо сейчас.</blockquote>'
        )
        cta = 'Попробуй бесплатно уже сейчас 🔥'

    text = (
        f'🔥 <b>TG Lead Wareon</b> — Автопоиск клиентов\n\n'
        f'<b>{name}, привет</b> 💬\n\n'
        f'Ты здесь, потому что хочешь клиентов — а не потому что любишь '
        f'рассылки. Поэтому коротко, как это работает 👇\n\n'
        f'<blockquote>Система поиска клиентов в Telegram — без программ, '
        f'технических знаний и ручного труда.</blockquote>\n'
        f'Подключаешь аккаунт, указываешь, где сидит твоя аудитория — '
        f'и бот делает остальное сам:\n'
        f'• собирает базу из открытых чатов и каналов\n'
        f'• пишет каждому персонально\n'
        f'• имитирует живое общение, чтобы не улетать в бан\n\n'
        f'<b>Не спам. Не один шаблон на всех.</b>\n'
        f'AI-тексты с подменой слов — каждое сообщение выглядит '
        f'написанным вручную.\n\n'
        f'<b>Что внутри</b>\n'
        f'🌐 <b>Умные авторассылки</b> — стабильно и безопасно\n'
        f'👤 <b>Автосбор аудитории</b> — по номеру или импортом сессии\n'
        f'✍️ <b>Персонализация</b> — обращение к каждому лично\n'
        f'🤖 <b>Сотни AI-шаблонов</b> — тексты под любую нишу\n'
        f'📊 <b>Детальный счётчик</b> — статистика доставки в реальном времени\n'
        f'⏯ <b>Пауза и стоп</b> — управление рассылкой в один клик\n'
        f'🧬 <b>Имитация человека</b> — задержки, «печатает», живой темп\n\n'
        f'{status_ln}\n'
        f'{cta}'
    )
    if edit_call:
